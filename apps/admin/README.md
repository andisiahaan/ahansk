...
- **Port Pengembangan**: `10313` (`turbo run dev`). URL aplikasi tidak menggunakan prefix `/admin/`.
...
```
Atau jalankan seluruh monorepo secara serentak via `pnpm run dev` dari root, lalu buka [http://localhost:10313](http://localhost:10313).

## 📋 Environment Variables (`.env.local`)
Salin dari `.env.example`:
```bash
NEXT_PUBLIC_API_URL=http://localhost:10311
NEXT_PUBLIC_FRONTEND_URL=http://localhost:10312
